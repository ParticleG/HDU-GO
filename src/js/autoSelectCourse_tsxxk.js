			...
			document.querySelectorAll('[type=checkbox]').forEach(item => {
				if(item.checked) {
					...
					courseListJson[item.parentElement.parentElement.children[3].innerText + item.parentElement.parentElement.children[4].innerText + item.parentElement.parentElement.children[5].innerText] = `<tr>
						...
						<td>
							<a onclick="removeCourse('` + item.parentElement.parentElement.children[3].innerText + item.parentElement.parentElement.children[4].innerText + item.parentElement.parentElement.children[5].innerText + `')" href="javascript:void(0);"> 移除 </a>
						</td>
					</tr>`;
					localStorage.setItem("courseListJson",JSON.stringify(courseListJson));
				}
		...
		document.querySelectorAll('[type=checkbox]').forEach(item => {
			if(checkBoxPatten.test(item.id)) {
				for (let i = 1; i < courseListArray.length; i++) {
					if ((item.parentElement.parentElement.children[2].innerText == courseListArray[i].querySelectorAll("td")[0].innerText) && (item.parentElement.parentElement.children[4].innerText == courseListArray[i].querySelectorAll("td")[2].innerText) && (item.parentElement.parentElement.children[5].innerText == courseListArray[i].querySelectorAll("td")[7].innerText)) {
						item.checked = true;
					}
				}
			}
